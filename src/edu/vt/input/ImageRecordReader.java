...
public class ImageRecordReader extends RecordReader<Text, Image> {
	...
	public void initialize(InputSplit genericSplit, TaskAttemptContext context)
		...
		fileIn.readFully(b);
		image = new Image(cvDecodeImage(
				cvMat(1, b.length, CV_8UC1, new BytePointer(b)), iscolor));
	...
	private Image getSubWindow() {
		...
		IplImage img2 = cvCreateImage(cvSize(roi.width(), roi.height()),
	...
	private void getConfig(Configuration conf) {
		...
		// Ensure that value is not negative
		sizePixel = conf.getInt("mapreduce.imagerecordreader.windowsizepixel",
				Integer.MAX_VALUE);
		if (sizePixel < 0) {
			sizePixel = 0;
		}
		...
		byPixel = conf.getBoolean("mapreduce.imagerecordreader.windowbypixel",
				false);
	}
	...
	private WindowInfo createWindow() {
		...
		if (window.getParentYOffset() + borderPixel + window.getHeight() < window
				.getParentHeight()) {
			bottom = borderPixel;
		}
	...
	private void calculateSplitInfo() {
		if (byPixel) {
			...
			totalXSplits = (int) Math.ceil(image.getWidth()
					/ Math.min(xSplitPixels, image.getWidth()));
			totalYSplits = (int) Math.ceil(image.getHeight()
					/ Math.min(ySplitPixels, image.getHeight()));
		} else {
			xSplitPixels = (int) (image.getWidth() * (sizePercent / 100.0));
			ySplitPixels = (int) (image.getHeight() * (sizePercent / 100.0));
			totalXSplits = (int) Math.ceil(image.getWidth()
					/ (double) Math.min(xSplitPixels, image.getWidth()));
			totalYSplits = (int) Math.ceil(image.getHeight()
					/ (double) Math.min(ySplitPixels, image.getHeight()));
		}
	}
}
